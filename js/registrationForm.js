...
registerButton.addEventListener("click", (e) => {
    ...
    const dateDob = new Date(dob.value) ;
    let age = joindate.getFullYear() - dateDob.getFullYear();
    const monthDiff = joindate.getMonth() - dateDob.getMonth();
    if (monthDiff < 0 || (monthDiff === 0 && joindate.getDate() < dateDob.getDate())) {
        age--;
    }
    let data = {};
    ...
    data.joiningTime = joindate.getDate() +'-' + joindate.getMonth() +'-' + joindate.getFullYear(); 

    if (data.fname.length < 2 || data.adhaar.length < 2 || data.address.length < 2 || data.mobile.length < 2 || age<18 ) {
        data = {};
        alert("invalid info .");
        return;
    }
